...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    num_training_steps_per_epoch=None, update_freq=None, num_epochs=None,
                    train_head_only=False, wandb_run=None, args=None):
    model.train(True)
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(metric_logger.log_every(data_loader, print_freq, num_epochs, epoch, ipe, header)):
        ...
        if it % 10 == 0 and utils.is_main_process() and wandb_run is not None:
            wandb_run.log({"train/loss": loss_value, "train/lr": optimizer.param_groups[-1]["lr"]})
...
@torch.no_grad()
def validation_one_epoch(data_loader, model, device, fp32=False, save_preds_path=None):
    ...
    softmaxes = []
    labels = []
    for batch in metric_logger.log_every(data_loader, 10, 1, 0, ipe, header=header):
        videos = batch[0]
        target = batch[1]
        ...
        softmaxes.append(torch.softmax(output, dim=1))
    ...
    # print("Before concat", softmaxes.shape)
    softmaxes = torch.cat(softmaxes_list)
    labels = torch.cat(labels_list)
    # print("After concat", softmaxes.shape)

    ece = compute_ece(softmaxes.cpu(), labels.cpu())
...
def merge(eval_path, num_tasks):
    ...
    for x in range(num_tasks):
        ...
        for line in lines:
            line = line.strip()
            name = line.rsplit('[', maxsplit=1)[0]
            label = line.rsplit(']',maxsplit=1)[1].split(' ')[1]
            chunk_nb = line.rsplit(']',maxsplit=1)[1].split(' ')[2]
            split_nb = line.rsplit(']',maxsplit=1)[1].split(' ')[3]
            data = np.fromstring(line.rsplit('[',maxsplit=1)[1].rsplit(']',maxsplit=1)[0], dtype=np.float, sep=',')
            data = softmax(data)
