...
class Transformer(nn.Module):
    def __init__(
            ...
            clip_return_layers=[6,7,8,9,10,11], clip_return_interval=1,
            return_cls=False,
        ):
        super().__init__()
        self.layers = layers
        ...
        # self.return_index = []
        # for i in range(clip_return_layers):
    ...
    def forward(self, x):
        ...
        for idx, blk in enumerate(self.resblocks):
            ...
            if idx in self.return_index:
                z.append(x)
            if idx == self.layers - 1 and self.return_cls:
                cls = x[0, :, :]
        x = torch.stack(z)
        return x, attn, cls
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        return_attn=False, clip_return_layers=[6,7,8,9,10,11], clip_return_interval=1,
        return_cls=False,
    ):
        super().__init__()
        ...
        print(f'Return Layer: {clip_return_layers}')
    ...
    def forward(self, x, mask=None):
        ...
        x, attn, cls_outputs = self.transformer(x)
        ...
        if self.return_attn:
            if self.return_cls:
                return x, attn[:, 0, 1:], cls_outputs
            else:
                return x, attn[:, 0, 1:]
        else:
            if self.return_cls:
                return x, cls_outputs
            else:
                return x
...
def clip_b16(
    ...
    return_attn=False, center=True, clip_return_layers=[6,7,8,9,10,11],
    clip_return_interval=1, return_cls=False,
):
...
if __name__ == '__main__':
    ...
    model = clip_ml_b16(pretrained=True, kernel_size=1, return_attn=False, clip_return_layers=1)
    # print(model)
