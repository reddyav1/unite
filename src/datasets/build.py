...
# from .rawframe import RawFrameClsDataset
...
def build_pretraining_dataset(args, annotation_file, fraction=1.0):
    transform = DataAugmentationForVideoMAE(args)
    dataset = VideoMAE(
        root=None,
        setting=annotation_file,
        prefix=args.prefix,
        split=args.split,
        video_ext='mp4',
        is_color=True,
...
def build_dataset(is_train, test_mode, args, annotation_file=None):
    ...
    if args.data_set in [
        ...
        if is_train is True:
            mode = 'train'
            anno_path = args.ann_file_train
        elif test_mode is True:
            mode = 'test'
            anno_path = args.ann_file_test
        else:  
            mode = 'validation'
            anno_path = args.ann_file_val
        
        # Override if annotation file provided by the caller explicitly
